 ...
 */
export default function formatRoutes(route) {
    const formatedRoute = {
        route: "",
    ...
    if (routePathSplitted.length !== 2) {
        throw new Error(`There cannot be more than one '/app/routes/' in path of your project. For more information, see our documentation ${joor.docs}/routes`);
    }
    ...
    for (let i in routePath) {
        ...
        if (routePath[index].match(dynamicRouteRegEx)) {
            if (index !== routePath.length - 1) {
                throw new Error(`You can only use any other end points inside dynamic endpoint. For more information, see our documentation ${joor.docs}/routes `);
            }
        }
    }
    ...
    if (routePath.startsWith("/web")) {
        ...
        if (routePath.startsWith("/api")) {
            throw new Error(`The route api cannot be used inside web folder. For more information, see our documentation ${joor.docs}/routes`);
        }
    }
