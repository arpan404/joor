...
class Configuration {
  ...
  /**
   * Sets the  needed configuration data to environment variables.
   * This env values are used in the application by other modules and packages.
   */
  private setConfigToEnv(): void {
    ...
    if (Configuration.configData?.env?.values) {
      ...
      keys.forEach((key) => {
        process.env[key] = Configuration.configData?.env?.values?.[key];
      });
    }
  }
  /**
