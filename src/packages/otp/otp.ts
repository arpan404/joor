...
export const otp = {
     ...
     */
    alphanumeric: {
        generate: (length: number) => generate(validateLength(length), '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz')
    },
     ...
     */
    numeric: {
        generate: (length: number) => generate(validateLength(length), '0123456789')
    },
     ...
     */
    alphabetic: {
        generate: (length: number) => generate(validateLength(length), 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz')
    },
    ...
    custom: {
        generate: (length: number, charSet: string) => {
            ...
            if (typeof charSet !== 'string' || charSet.length < 10) {
                logger.warn(`Invalid character set: "${charSet}". Using default character set: '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.`);
                charSet = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'; // Default character set
            }

            return generate(validatedLength, charSet);
        }
    }
};
