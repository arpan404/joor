...
Generated native dispatcher, Bun, Deno, Cloudflare, Next.js, Vercel, and Netlify entrypoints also export typed factory helpers such as `createFetchFor<Request>()` and `.joor/next`'s `createHandlersFor<Context, Request>()` so generated handlers can preserve extended request types.
